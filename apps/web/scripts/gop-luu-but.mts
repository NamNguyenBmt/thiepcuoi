/**
 * Cho hai thiệp dùng chung một sổ lưu bút.
 *
 ...
 * ở thiệp nào cũng vào chung một sổ, mở thiệp nào cũng thấy đủ. Phản hồi RSVP
 * và bộ đếm tim vẫn tách riêng theo thiệp.
 *
...
import { getInviteBySlug, listWishes, shareWishbook } from '../lib/db';
...
console.log(`Gốc  ${slugGoc}: ${truocGoc.length} lời chúc`);
console.log(`Mượn ${slugMuon}: ${daChung ? 'đã dùng chung sổ gốc' : `${truocMuon.length} lời chúc`}`);
...
const khop = sauGoc.length === sauMuon.length && sauGoc.every((w, i) => w.id === sauMuon[i]!.id);
console.log(`\n✓ Đã dồn ${kq.moved} lời chúc. Sổ chung giờ có ${sauGoc.length} lời chúc.`);
console.log(khop ? '✓ Hai thiệp đọc ra cùng một sổ.' : '✗ Đọc lại hai thiệp ra hai sổ khác nhau — xem lại!');
